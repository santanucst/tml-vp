...
export class InvoiceAddEditComponent implements OnInit {
  ...
  public creditNoteLength: number = this.localStorageService.dbSettings.drcrnoteNumber;
  ...
  public invoiceDateError: boolean = false;
  public invoiceDateErrorDesc: string = "";
  ...
  private getPONosWs(code: string) {
    ...
    this.invoiceAddEditDataService.getPONosVal(code).
      subscribe(res => {
  ...
  private getItemDetailsWS(poNo: string) {
    this.invoiceAddEditDataService.getItemDetVal(this.code, poNo).
      subscribe(res => {
  ...
  private getVendorDetValWS(vendorCode: string) {
    ...
    this.invoiceAddEditDataService.getVendorDetVal('', vendorCode).
      subscribe(res => {
  ...
  onVenderCodeKeyUp(vendorCode: string){
    if(!vendorCode){
      this.vendorCodeError = false;
  ...
  onChangeInvoiceDate(invoiceDateParam: string){
    let invoiceDate: string = invoiceDateParam;
    this.invoiceDateError = false;
    this.invoiceDateErrorDesc = "";
    //formatting the current date
    let date = new Date();
    invoiceDate = this.datePipe.transform(invoiceDate, 'yyyy-MM-dd');
    this.poDate = this.datePipe.transform(this.poDate, 'yyyy-MM-dd');
    if(invoiceDate < this.poDate){
      this.invoiceDateError = true;
      this.invoiceDateErrorDesc = "Invoice Date can't be less than P.O Date";
    }
  ...
  public invoiceAddSubmit() {
    ...
    this.busySpinner.submitBusy = true;
    this.updateBusySpinner();
    this.invoiceAddEditDataService.transactionAddSubmit(invoiceTranDet).
      subscribe(res => {
